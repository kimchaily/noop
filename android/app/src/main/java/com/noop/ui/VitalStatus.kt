...
object VitalStatus {
    ...
    /**
     * The wellness COLOUR for a vital (green→amber→red by state), or [fallback] when the metric isn't a
     * banded vital or has no reading yet. [fallback] is the surface's old identity tint, so a non-vital
     * tile (Steps, Calories, the scores) and a no-data vital keep their existing colour.
     */
    fun color(key: String, day: DailyMetric?, days: List<DailyMetric>, fallback: Color): Color =
        goodness(key, day, days)?.let { Palette.goodnessColor(it) } ?: fallback
}
