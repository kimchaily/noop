...
object Baselines {
     ...
     *
     * The analytics layer is Context-free, so the caller passes in a simple putLong callback.
     * Epochs are stored as whole seconds in a Long, matching the "epoch SECONDS" the keys document
     * and the iOS UserDefaults values byte-for-byte.
     */
    fun recalibrateRecoveryBaselines(putLong: (String, Long) -> Unit, nowSeconds: Long) {
        putLong(hrvBaselineEpochKey, nowSeconds)
        putLong(recoveryBaselineEpochKey, nowSeconds)
    }
}
