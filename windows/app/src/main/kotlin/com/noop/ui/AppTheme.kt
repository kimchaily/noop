...
//
// Each family is a cohesive design with its own canvas, accent, per-domain "colour worlds", metal
// ramp, scenic sky tint and typeface — and each available in BOTH light and dark. The Light/Dark/
// System toggle chooses the light or dark variant OF THE SELECTED FAMILY.
//
// The ~70-token `PaletteTokens` contract is frozen (every screen reads `Palette.X`). A new theme is
// built by `.copy()`-ing the WHOOP base tokens and overriding only its IDENTITY tokens. The
// functional DATA encodings (recovery red->green, HR zones, sleep stages, status) are inherited so
// every chart stays as legible + consistent as the default.
...
//
// Mirrors [AppearancePrefs]/[ChartStylePrefs]: persisted via [NoopPrefs] and mirrored in snapshot
// state so switching family re-resolves every `Palette.*` read AND the app typeface live, with no
// flash. [load] runs once before first composition.
...
object ThemePrefs {
    ...
    fun load() {
        family = AppTheme.fromStorage(NoopPrefs.getString(KEY, AppTheme.DEFAULT.storageValue))
    }
